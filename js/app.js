...
class UIController {
    ...
    attachEventListeners() {
        ...
        // Quick action buttons - use unified bank transfer modal
    ...
    renderTransactions(container, transactions) {
        ...
        container.innerHTML = transactions.map(trans => {
            ...
            const icon = trans.type === 'savings' ? 'arrow-down' : 'arrow-up';
            const iconClass = trans.type === 'savings' ? 'savings' : 'payment';
    ...
    openBankTransferModal(type) {
        ...
        if (type === 'payment') {
            ...
            amountInput.value = this.appState.getLoanRemaining();
    ...
    handleSimplePayment() {
        ...
        if (remaining <= 0) {
            ...
            // Clear notifications after showing success
            setTimeout(() => {
                successOverlay.classList.remove('active');
                // Clear notifications
                this.appState.notifications = [];
